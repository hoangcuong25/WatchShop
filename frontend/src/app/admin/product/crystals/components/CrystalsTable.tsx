...
export default function CrystalsTable({ crystals, setIsEditDialogOpen, setIsDeleteDialogOpen }: { crystals: CrystalType[], setIsEditDialogOpen: (value: boolean) => void, setIsDeleteDialogOpen: (value: boolean) => void }) {
    return (
                    ...
                    <tbody className="bg-white dark:bg-slate-800 divide-y divide-slate-200 dark:divide-slate-700">
                        {crystals.map((crystal) => (
                            <tr key={crystal.id} className="hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
                                ...
                                <td className="hidden md:table-cell px-3 sm:px-6 py-4 whitespace-nowrap text-sm text-slate-500 dark:text-slate-400">
                                    {crystal.updatedAt ? new Date(crystal.updatedAt).toLocaleDateString() : ''}
                                </td>
